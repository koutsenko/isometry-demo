(function () {
    ...
    */
    ...
    function fillScene() {
        ...
        logger.log('В sceneEl создан gridEl с количеством плиток: ' + GRID_SIZE_X + 'x' + GRID_SIZE_Y);
    ...
    function fillSceneWithHeightFix() {
        ...
        logger.log('На gridEl навешен доп. CSS класс ' + SOLUTION_HEIGHT_FIX_CLASS);
    ...
    function fillSceneWithPerspectiveFix() {
        ...
        logger.log('На gridEl навешен доп. CSS класс ' + SOLUTION_PERSPECTIVE_FIX_CLASS);
    ...
    function fillSceneWithPositionsCalc() {
        ...
        logger.log('В sceneEl созданы gridEl и gridWrapper, количество плиток: ' + GRID_SIZE_X + 'x' + GRID_SIZE_Y);
        ...
        logger.log('Рассчитан gridEl boundingBox: ' + JSON.stringify(roundObjValues({ maxX, maxY, minX, minY })));
        ...
        logger.log(`Рассчитаны смещения: isoOffset.x=${Math.round(isoOffset.x)}, isoOffset.y=${Math.round(isoOffset.y)}`);

        // Осуществляем сдвиг gridEl
        gridEl.style.left = px(isoOffset.x);
        gridEl.style.top = px(isoOffset.y);
        logger.log(`Применены смещения к gridEl: left=${gridEl.style.left}, top=${gridEl.style.top}`);
